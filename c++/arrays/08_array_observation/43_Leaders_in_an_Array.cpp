...
Given an integer array nums, return a list of all the leaders in the 
array.
...
Example 1
    ...
    2 is the rightmost element, 3 is the largest element in the index 
    range [3, 5], 5 is the largest element in the index range [2, 5]
...
Example 2
    ...
    -5 is the rightmost element, -4 is the largest element in the index 
    range [4, 5], 1 is the largest element in the index range [3, 5] and 
...
vector<int> leaders(vector<int>& nums) {
    ...
    for(int i=n-2; i>=0; i--) {
        if(nums[i] > maxRight) { // some problems have >=
            maxRight = nums[i];
            ans.push_back(maxRight);
        }
    }
